...
> = {
  components: { itemType: "registry:component", fileType: "registry:component", targetDir: "components" },
  hooks:      { itemType: "registry:hook",       fileType: "registry:hook",       targetDir: "hooks" },
  lib:        { itemType: "registry:lib",         fileType: "registry:lib",         targetDir: "lib" },
  types:      { itemType: "registry:lib",         fileType: "registry:lib",         targetDir: "types" },
...
type RegistryItemType =
  | "registry:component" | "registry:block" | "registry:ui"
  | "registry:hook" | "registry:lib" | "registry:page";

type RegistryFileType =
  | "registry:component" | "registry:style" | "registry:hook"
  | "registry:lib" | "registry:page" | "registry:file";
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (const target of positional) {
    if (target.includes("/")) {
      ...
      if (!found) {
        console.warn(`⚠  Target "${target}" not found in any category — skipping.`);
      }
    }
  }
...
function loadExistingRegistry(): Registry | null {
  ...
  try {
    ...
    console.warn("⚠  Existing registry.json could not be parsed — it will be overwritten.");
    return null;
  }
}
...
function loadPackageJson(): PackageJson {
  if (!fs.existsSync(PACKAGE_JSON)) {
    console.warn("⚠  No package.json found — dependency versions will be omitted.");
    return {};
  }
...
function analyzeFile(source: string, packageMap: Map<string, string>): FileDeps {
  ...
  for (const importPath of extractImportPaths(source)) {
    ...
    if (shadcnName) { registry.add(shadcnName); continue; }
...
function inferFileType(filePath: string, categoryDefault: RegistryFileType): RegistryFileType {
  ...
  if (base === "utils" || base === "helpers" || base === "lib") return "registry:lib";
...
function processEntity(
  ...
  if (sourceFiles.length === 0) {
    console.warn(`  ⚠  Skipping "${categoryName}/${entityName}" — no source files found.`);
    return null;
  }
  ...
  const registryFiles: RegistryFile[] = sourceFiles.map((absPath) => {
    ...
    if (!absPath.endsWith(".css")) {
      const { npm, registry, unresolved } = analyzeFile(content, packageMap);
...
function main() {
  ...
  if (existingRegistry) {
    console.log(`📄  Found existing registry.json with ${existingItemMap.size} item(s)`);
  }
  ...
  const activeCategories = Object.keys(ENTITY_CATEGORIES).filter((name) => presentDirs.has(name));

  if (activeCategories.length === 0) {
    console.warn("⚠  No recognised category directories found. Expected:", Object.keys(ENTITY_CATEGORIES).join(", "));
    process.exit(0);
  }
  ...
  for (const categoryName of activeCategories) {
    ...
    for (const entityName of entityNames) {
      ...
      const item = processEntity(categoryName, entityName, entityDir, category, packageMap);
      if (!item) continue;
      ...
      const depsLog = [
        ...
        item.registryDependencies?.length ? `registry: ${item.registryDependencies.join(", ")}` : "",
      ].filter(Boolean).join(" | ");

      if (alreadyExists) {
        console.log(`  ↺  ${entityName}  (updated)${depsLog ? `  — ${depsLog}` : ""}`);
        summary.updated++;
      } else {
        console.log(`  ✓  ${entityName}  (added)${depsLog ? `  — ${depsLog}` : ""}`);
        summary.added++;
      }
    }
  }
